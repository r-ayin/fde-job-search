# -*- coding: utf-8 -*-
"""只收集 FDE 岗位（岗位名必须含 FDE/前沿部署/前线部署/前向部署）。"""
...
FDE_KW = ['fde', '前沿部署', '前线部署', '前向部署', 'Forward Deployed', '部署工程师']
...
# 已有 eid
existing = set()
for f in ['jd_final.json','jd_search_final.json','jd_missing.json','chatlist_full.json','send_results_v5.json','send_results_v4.json']:
    try:
...
for kw in FDE_KW:
    ...
    for page in range(1, 9):
        ...
        if not d or d.get('err') or d.get('code'):
            print(f"[{kw} p{page}] {str(d)[:70]}", flush=True)
        ...
        for row in d.get('rows') or []:
            ...
            # 只保留 FDE 类岗位
            low = jn.lower()
            if not any(m in low for m in FDE_MARK): continue
