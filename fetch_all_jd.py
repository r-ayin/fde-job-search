# -*- coding: utf-8 -*-
"""抓取搜索页所有岗位的 JD 正文（排除实习/日结），为评分提供依据。"""
...
print(f"非实习岗位 {len(todo)} 个（总 {len(cards)}）", flush=True)

results = []
if os.path.exists('search_jd_all.json'):
    results = json.load(open('search_jd_all.json', encoding='utf-8'))
done = {x['eid'] for x in results if x.get('ok')}
...
for i, c in enumerate(todo):
    ...
    try:
        ...
        d.update({'eid': eid, 'listName': c.get('name'), 'company': c.get('company'),
                  'listSalary': c.get('salary'), 'listTags': c.get('tags'), 'area': c.get('area')})
        results.append(d)
        print(f"[{i+1}/{len(todo)}] {'OK ' if d.get('ok') else 'FAIL'} {c.get('company')} | {c.get('name')[:28]} | desc={len(d.get('desc') or '')}", flush=True)
    except Exception as e:
        print(f"[{i+1}] ERR {str(e)[:80]}", flush=True)
        results.append({'eid': eid, 'listName': c.get('name'), 'company': c.get('company'), 'ok': False})
    if (i+1) % 5 == 0:
        json.dump(results, open('search_jd_all.json','w',encoding='utf-8'), ensure_ascii=False, indent=1)
